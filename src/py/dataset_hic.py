...
class SeqDataset:
  def __init__(self, data_dir, split_label, batch_size, shuffle_buffer=128,
               hic_length = 25000, cell_id=None, seq_length_crop=None, mode='eval', tfr_pattern=None):
    ...
    if self.tfr_pattern is None:
      self.tfr_path = '%s/tfrecords_hic/%s-*.tfr' % (self.data_dir, self.split_label)
  ...
  def generate_parser(self, raw=False):
    def parse_proto(example_protos):
      ...
      features = {
        ...
        TFR_INPUT_HIC: tf.io.FixedLenFeature([], tf.string),
      ...
      # decode hic
      hic = tf.io.decode_raw(parsed_features[TFR_INPUT_HIC], tf.uint8)
      if not raw:
        hic= tf.reshape(hic, [self.hic_length, self.seq_depth])
        hic = tf.cast(hic, tf.float32)
      
      # decode targets
      # extract only one target (one index)
