...
# =========================================================================
"""SeqNN trainer for HiCBoost. Based on Calcio 2019 Basenji project."""
# =========================================================================
import time
...
class Trainer:
  ...
  def fit_keras(self, seqnn_model):
    ...
    seqnn_model.save('%s/last_saved'%self.out_dir)
  ...
  def fit_tape(self, seqnn_model):
    ...
    # metrics
    num_targets = seqnn_model.output_shape[-1]
    print(num_targets)
    ...
    if self.strategy is None:
      ...
      def train_step(x, y):
        with tf.GradientTape() as tape:
          pred = seqnn_model(x, training=True)
          loss = self.loss_fn(y, pred) + sum(seqnn_model.losses)
        train_loss(loss)
        train_r(y, pred)
        train_r2(y, pred)
        gradients = tape.gradient(loss, seqnn_model.trainable_variables)
        self.optimizer.apply_gradients(zip(gradients, seqnn_model.trainable_variables))

      @tf.function
      def eval_step(x, y):
        pred = seqnn_model(x, training=False)
        loss = self.loss_fn(y, pred) + sum(seqnn_model.losses)
        valid_loss(loss)
        valid_r(y, pred)
        valid_r2(y, pred)
      ...
      def train_step(x, y):
        with tf.GradientTape() as tape:
          ...
          loss_batch = tf.reduce_mean(loss_batch_len, axis=-1)
          loss = tf.reduce_sum(loss_batch) / self.batch_size
          loss += sum(seqnn_model.losses) / self.num_gpu
        train_r(y, pred)
        train_r2(y, pred)
        gradients = tape.gradient(loss, seqnn_model.trainable_variables)
        self.optimizer.apply_gradients(zip(gradients, seqnn_model.trainable_variables))
    ...
    # checkpoint manager
    ckpt = tf.train.Checkpoint(model=seqnn_model, optimizer=self.optimizer)
