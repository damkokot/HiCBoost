...
class GELU_DENSE(tf.keras.layers.Layer):
  def __init__(self, **kwargs):
    super(GELU_DENSE, self).__init__(**kwargs)
  def call(self, x):
    # return tf.keras.activations.sigmoid(1.702 * x) * x
    return tf.keras.activations.sigmoid(tf.constant(1.702) * x) * x

class GELU_HEAD(tf.keras.layers.Layer):
  def __init__(self, **kwargs):
    super(GELU_HEAD, self).__init__(**kwargs)
  def call(self, x):
    # return tf.keras.activations.sigmoid(1.702 * x) * x
    return tf.keras.activations.sigmoid(tf.constant(1.702) * x) * x
...
class EnsembleReverseComplement(tf.keras.layers.Layer):
  ...
  def __init__(self, **kwargs):
    super(EnsembleReverseComplement, self).__init__(**kwargs)
...
# same, but for HiC
class StochasticReverseComplementHic(tf.keras.layers.Layer):
  """Stochastically reverse complement a one hot encoded DNA sequence."""
  def __init__(self, **kwargs):
    super(StochasticReverseComplementHic, self).__init__(**kwargs)
  def call(self, seq_1hot, training=None):
    if training:
      rc_seq_1hot = tf.gather(seq_1hot, [3, 2, 1, 0], axis=-1)
      rc_seq_1hot = tf.reverse(rc_seq_1hot, axis=[1])
...
class SwitchReverse(tf.keras.layers.Layer):
  ...
  def __init__(self, strand_pair=None, **kwargs):
...
# for HiC
class SwitchReverseHic(tf.keras.layers.Layer):
  """Reverse predictions if the inputs were reverse complemented."""
  def __init__(self, strand_pair=None, **kwargs):
    super(SwitchReverseHic, self).__init__(**kwargs)
    self.strand_pair = strand_pair
  def call(self, x_reverse):
    x = x_reverse[0]
    reverse = x_reverse[1]
    ...
    if xd == 3:
      ...
      raise ValueError('Cannot recognize SwitchReverseHic input dimensions %d.' % xd)
...
class SwitchReverseTriu(tf.keras.layers.Layer):
  def __init__(self, diagonal_offset, **kwargs):
    super(SwitchReverseTriu, self).__init__(**kwargs)
    self.diagonal_offset = diagonal_offset
...
class EnsembleShift(tf.keras.layers.Layer):
  ...
  def __init__(self, shifts=[0], pad='uniform', **kwargs):
    super(EnsembleShift, self).__init__(**kwargs)
    self.shifts = shifts
    self.pad = pad
...
# for HiC
class StochasticShiftHic(tf.keras.layers.Layer):
  """Stochastically shift a one hot encoded DNA sequence."""
  def __init__(self, shift_max=0, symmetric=True, pad='uniform', **kwargs):
    super(StochasticShiftHic, self).__init__(**kwargs)
    self.shift_max = shift_max
    self.symmetric = symmetric
    if self.symmetric:
      self.augment_shifts = tf.range(-self.shift_max, self.shift_max+1)
...
def activate(current, activation, verbose=False):
  ...
  elif activation == 'gelu_dense':
    current = GELU_DENSE()(current)
  elif activation == 'gelu_head':
    current = GELU_HEAD()(current)  
