...
################################################################################
def main():
  usage = 'usage: %prog [options] <fasta_file> <seqs_bed_file> <hic_data> <seqs_cov_dir> <tfr_file>'
  ...
  if len(args) != 5:
    ...
    hic_data = hicstraw.HiCFile(args[2])
    seqs_cov_dir = args[3]
    tfr_file = args[4]
  ...
  with tf.io.TFRecordWriter(tfr_file, tf_opts) as writer:
    for si in range(num_seqs):
      ...
      hic_1hots = []
      for msq_hic in hic_seq:
        seq_dna_hic = fetch_dna(fasta_open, msq_hic.chr, msq_hic.start, msq_hic.end)
        seq_1hot_hic = dna_1hot(seq_dna_hic,n_uniform=False, n_sample=False)
        hic_1hots.append(seq_1hot_hic)
        
      # hash to bytes
      features_dict = {
        'sequence': feature_bytes(seq_1hot),
        'hic': feature_bytes(np.asarray(hic_1hots)),
